...
.setTitle('Oyun & Sohbet Sunucu Şablonu')
.setColor('#0044ff')
.setThumbnail('')
.setFooter(`Ping: ${client.ws.ping.toFixed(0)}`, client.user.avatarURL({dynamic: true}))
.setDescription(`${message.author} \<a:duyuru3:872720101177196564> **Sunucu** Şablonunun Kurulmasını Onaylıyor musunuz?

> \<a:duyuru3:872720101177196564>  **Not:** Bazı Kanallar Silinmemiş veya Eksik Gözükebilir,Discord'a Yeniden Giriş Yaparsanız Düzelecektir.`)).then(resulter => {
...
message.guild.channels.create('🎤 | Ses Teyit ¹', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(5));
message.guild.channels.create('🎤 | Ses Teyit ²', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(5));
message.guild.channels.create('🎤 | Ses Teyit ³', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(5));
});
...
message.guild.channels.create('📥・sunucu-sayaç', {type: 'text'}).then(c => c.setParent(parent.id));
...
message.guild.channels.create('🍺 ・ Havaalanı', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(15));
message.guild.channels.create('💧 ・ Bahçe', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(15));
message.guild.channels.create('🌲・ Restaurant', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(15));
message.guild.channels.create('🍻 ・Muğla', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(15));
});

message.guild.channels.create('●▬▬▬▬๑「🎵」๑▬▬▬▬●', {type: 'category'}).then(parent => {
message.guild.channels.create('🎵 ・ Meydan', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(10));
message.guild.channels.create('🎤 ・ Canlı Müzik', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(15));
message.guild.channels.create('🔓 ・ Konservatuar', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(10));
message.guild.channels.create('💫・ Müzikal', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(10));
});
...
message.guild.channels.create('●▬▬▬▬๑「🎰」๑▬▬▬▬●', {type: 'category'}).then(parent => {
message.guild.channels.create('👥・oyun-arkadaş-arama', {type: 'text'}).then(c => c.setParent(parent.id));
