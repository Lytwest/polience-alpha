...
exports.run = async (client, message, args) => {
  ...
  if(!message.mentions.roles.first()) return message.channel.send(new Discord.MessageEmbed().setColor('#0044ff').setTitle('Bir Hata Oluştu!').setDescription('\<a:silah:872719436799442964> Rol Etiketleyiniz.'));
  let mentionRole = message.mentions.roles.first();
  data.set(`jail.karantinarole.${message.guild.id}`, mentionRole.id);
  message.channel.send(new Discord.MessageEmbed().setTitle('Sistemi Başarıyla Ayarladın!').setDescription(`\<a:nokta:830441156151214090> Jail Karantina Rolünü ${mentionRole} Olarak Seçtiniz!`));
